...
export default function Home() {
    ...
    // Arena Settings
    const ARENAS = [
        { id: 10, name: 'NOVATO', icon: '­ƒ¬Ö', color: '#4caf50', entry: 10, prize: 20 },
        { id: 100, name: 'AVANZADO', icon: '­ƒöÑ', color: '#ff9800', entry: 100, prize: 200 },
        { id: 500, name: 'ELITE', icon: '­ƒÆÄ', color: '#2196f3', entry: 500, prize: 1000 },
        { id: 1000, name: 'LEYENDA', icon: '­ƒææ', color: '#e91e63', entry: 1000, prize: 2000 }
    ];
    ...
    const RANKS = [
        { id: 'BRONCE', name: 'BRONCE', icon: '­ƒÑë', color: '#cd7f32', minRp: 0, maxRp: 100, stake: 1 },
        { id: 'PLATA', name: 'PLATA', icon: '­ƒÑê', color: '#c0c0c0', minRp: 101, maxRp: 300, stake: 2 },
        { id: 'ORO', name: 'ORO', icon: '­ƒÑç', color: '#ffd700', minRp: 301, maxRp: 600, stake: 5 },
        { id: 'PLATINO', name: 'PLATINO', icon: '­ƒÆá', color: '#e5e4e2', minRp: 601, maxRp: 1000, stake: 10 },
        { id: 'DIAMANTE', name: 'DIAMANTE', icon: '­ƒÆÄ', color: '#b9f2ff', minRp: 1001, maxRp: 2000, stake: 25 },
        { id: 'LEYENDA', name: 'LEYENDA', icon: '­ƒææ', color: '#ff00ff', minRp: 2001, maxRp: 999999, stake: 50 }
    ];
    ...
    useEffect(() => {
        ...
        const connectSocket = async () => {
            ...
            socketIo.on('purchaseError', (error: string) => {
                ...
                alert(`Error en la compra: ${error}. Por favor, verifica tu conexi├│n o base de datos.`);
            });
        ...
        <>
            ...
            <div className="main-header">
                ...
                <div
                    ...
                    title="Open Gem Shop"
                >
                    <span className="economy-icon">­ƒÆÄ</span>
                ...
                >
                    <span className="economy-icon">­ƒ¬Ö</span>
                ...
                <div className="leaderboard-overlay" style={{ zIndex: 10001 }}>
                    <div className="leaderboard-card" style={{ maxWidth: '400px' }}>
                        <div className="leaderboard-header">
                            <div className="leaderboard-logo">ÔÜÖ´©Å</div>
                            <h2 className="leaderboard-title">SETTINGS</h2>
                            <button className="close-btn" onClick={() => setShowSettings(false)}>├ù</button>
                        </div>
            ...
            {showCoinShop && (
                <div className="leaderboard-overlay" style={{ zIndex: 10002 }}>
                    <div className="leaderboard-card shop-card" style={{ maxWidth: '400px' }}>
                        <div className="leaderboard-header">
                            <div className="leaderboard-logo">­ƒ¬Ö</div>
                            <h2 className="leaderboard-title">COIN SHOP</h2>
                            <button className="close-btn" onClick={() => setShowCoinShop(false)}>├ù</button>
                        </div>
                        <div className="shop-grid">
                            {[10, 50, 100, 250, 500, 1000].map((amount) => (
                                ...
                                >
                                    <span className="shop-item-icon">­ƒ¬Ö</span>
            ...
            {showGemShop && (
                <div className="leaderboard-overlay" style={{ zIndex: 10003 }}>
                    <div className="leaderboard-card shop-card" style={{ maxWidth: '400px' }}>
                        <div className="leaderboard-header">
                            <div className="leaderboard-logo">­ƒÆÄ</div>
                            ...
                            <button className="close-btn" onClick={() => setShowGemShop(false)}>├ù</button>
                        </div>
                        <div className="shop-grid">
                            {[10, 50, 100, 250, 500, 1000].map((amount) => (
                                <button
                                ...
                                >
                                    <span className="shop-item-icon">­ƒÆÄ</span>
                                    <span className="shop-item-amount">+{amount}</span>
                                    <span className="shop-item-price">RELOAD</span>
                                </button>
                            ))}
            ...
            {showLeaderboard && (
                <div className="leaderboard-overlay">
                    <div className="leaderboard-card">
                        <div className="leaderboard-header">
                            <div className="leaderboard-logo">­ƒÅå</div>
                            <h2 className="leaderboard-title">HALL OF FAME</h2>
                            <button className="close-btn" onClick={() => setShowLeaderboard(false)}>├ù</button>
                        </div>
                    ...
                    <>
                        <div className="score-bar score-bar-left">
                            ...
                            <div className="score-avatar">
                                {user?.imageUrl ? <img src={user.imageUrl} className="avatar-img" alt="You" /> : <span style={{ fontSize: '1.5rem' }}>­ƒÿÄ</span>}
                            </div>
                        </div>
                        <div className="score-bar score-bar-right">
                            ...
                            <div className="score-avatar">
                                {opponentImageUrl ? <img src={opponentImageUrl} className="avatar-img" alt="Opponent" /> : <span style={{ fontSize: '1.5rem' }}>­ƒñû</span>}
                            </div>
                        </div>
                    </>
                )}
                        ...
                        {currentMatchStake && (
                            ...
                            >
                                ...
                                <span style={{ position: 'relative', zIndex: 1 }}>
                                    APUESTA: {currentMatchStake * 2} {gameMode === 'casual' ? '­ƒ¬Ö' : '­ƒÆÄ'}
                                </span>
                            </div>
                        )}
                        {gameState === 'playing' && (
                ...
                {(gameState === 'countdown' || gameState === 'playing' || gameState === 'roundResult') && (
                    <div className="hands-container">
                        <div className={`hand top ${showCollision ? 'collide-top' : ''}`}>
                            {gameState === 'roundResult' && opponentChoice ? CHOICE_EMOJIS[opponentChoice] : 'Ô£è'}
                        </div>
                        <div className={`hand bottom ${showCollision ? 'collide-bottom' : ''}`}>
                            {gameState === 'roundResult' && playerChoice ? CHOICE_EMOJIS[playerChoice] : 'Ô£è'}
                        </div>
                    </div>
                )}
                                        ...
                                        <div className="arena-grid">
                                            {(gameMode === 'casual' ? ARENAS : RANKED_ARENAS).map((arena) => (
                                                ...
                                                >
                                                    ...
                                                    <div className="arena-entry">{arena.entry} {gameMode === 'casual' ? '­ƒ¬Ö' : '­ƒÆÄ'}</div>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                    ...
                                    >
                                        ...
                                        <div style={{
                                            ...
                                            {[...Array(5)].map((_, i) => (
                                                ...
                                                >
                                                    {rewardData.type === 'rp' ? 'Ô¡É' : rewardData.type === 'gems' ? '­ƒÆÄ' : '­ƒ¬Ö'}
                                                </motion.div>
                                            ))}
                                        </div>
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <span style={{ marginLeft: '8px' }}>
                                                {rewardData.type === 'coins' ? '­ƒ¬Ö' : rewardData.type === 'gems' ? '­ƒÆÄ' : 'Ô¡É'}
                                            </span>
                                        </motion.div>
                                        <div style={{
                                            fontSize: '0.85rem',
